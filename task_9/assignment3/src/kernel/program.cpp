...
int ProgramManager::processCollector()
{
    // 清空Zoombie进程，返回被回收的数目
    ...
    ListItem *progressList_;
    ListItem *progressList_next;
    PCB *progressList_PCB;
    int releaseNum = 0; // 记录被释放的僵尸进程数


    progressList_ = this->allPrograms.head.next; // 从头开始遍历
    while (progressList_)
    {
        progressList_PCB = ListItem2PCB(progressList_, tagInAllList);
        progressList_next = progressList_->next;
        // 如果不是内核线程则参与讨论
        if (progressList_PCB->pageDirectoryAddress)
        {
            if (progressList_PCB->status == ProgramStatus::DEAD)
            {
                printf("[Kernal]OS has collected zoombie process[Pid: %d]\n", progressList_PCB->pid);
                releasePCB(progressList_PCB);
                releaseNum++;
            }
        }
        progressList_ = progressList_next;
    }
