...
void STDIO::print(uint x, uint y, uint8 c, uint8 color)
{
    ...
    uint pos = x * 80 + y;
    asm_system_call(4,(int)screen,pos,c,color);
}

void STDIO::print(uint8 c, uint8 color)
{
    uint cursor = getCursor();
    asm_system_call(4,(int)screen,cursor,c,color);
    cursor++;
    if (cursor == 25 * 80)
    {
        rollUp();
...
void STDIO::moveCursor(uint position)
{
    ...
    asm_system_call(2,0x3d4,0x0e);
    asm_system_call(2,0x3d5,temp);

    // 处理低8位
    temp = position & 0xff;
    asm_system_call(2,0x3d4,0x0f);
    asm_system_call(2,0x3d5,temp);
}
...
uint STDIO::getCursor()
{
    ...
    // 处理高8位
    asm_system_call(2,0x3d4,0x0e);
    asm_system_call(3,0x3d5,(int)&temp);
    pos = ((uint)temp) << 8;
...
void STDIO::rollUp()
{
    ...
    {
        asm_system_call(5,(int)screen,i,(int)&c,(int)&color);
        asm_system_call(4,(int)screen,i-80,c,color);
    }

    for (uint i = 24 * 80; i < length; ++i)
    {
        asm_system_call(4,(int)screen,i,' ',0x07);
    }
}
