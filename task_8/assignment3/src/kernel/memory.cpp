...
//         if (type == AddressPoolType::KERNEL)
//         {
//             swapAddr = kernelPhysical.swapOut();
//             releasePhysicalPages(AddressPoolType::KERNEL, swapAddr, 1);
//         }
//         else if (type == AddressPoolType::USER)
//         {
//             swapAddr = userPhysical.swapOut();
//             releasePhysicalPages(AddressPoolType::USER, swapAddr, 1);
//         }
//     }
...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count)
{
    ...
    if (!virtualAddress)
    {
        printf_error("[Virtual address is 0, Unsuccessful Allocated]\n");
        return 0;
    }
    ...
    for (int i = 0; i < count; ++i, vaddress += PAGE_SIZE)
    {
        ...
        if (!flag)
        {
            ...
            printf_error("[Unabundant phy-pages, Unsuccessful Allocated]\n");
            return 0;
        }
    }
    printf_warning("[Successful Allocated] Address 0x%x\n", virtualAddress);
...
int MemoryManager::allocateVirtualPages(enum AddressPoolType type, const int count)
{
    ...
    while (start == -1)
    {
        ...
        if (type == AddressPoolType::KERNEL)
        {
            swapAddr = kernelVirtual.swapOut();
            releasePages(AddressPoolType::KERNEL, swapAddr, 1);
        }
    }
...
bool MemoryManager::connectPhysicalVirtualPage(const int virtualAddress, const int physicalPageAddress)
{
    ...
    *pte = physicalPageAddress | 0x7;
    ...
    printf_warning("We have connect 0x%x and 0x%x\n",virtualAddress, physicalPageAddress);
    return true;
}
...
{
    printf_warning("[Releasing Pages...] Address 0x%x\n", virtualAddress);
